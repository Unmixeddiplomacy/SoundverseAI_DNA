          ...
          <div className="flex-1 space-y-2 sm:space-y-2.5 md:space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2 md:gap-3 lg:gap-4">
              ...
              <button type="button" onClick={() => {
                const defaultTags = ["Pop", "Electronic", "Indie"];
                setArtistData({
                  ...artistData,
                  tags: Array.from(new Set([...(artistData.tags || []), ...defaultTags]))
                });
              }}>
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {(artistData.tags || ["Pop", "Electronic", "Indie"]).map((tag: string, idx: number) => (
                <span key={tag} className="bg-[#232323] text-white px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
                  {tag}
                  <button type="button" className="ml-1 text-[#8F6FFF] hover:text-red-400" onClick={() => {
                    setArtistData({
                      ...artistData,
                      tags: (artistData.tags || ["Pop", "Electronic", "Indie"]).filter((t: string) => t !== tag)
                    });
                  }}>
                    &times;
                  </button>
