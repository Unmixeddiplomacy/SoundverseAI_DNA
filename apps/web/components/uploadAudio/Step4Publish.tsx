...
export default function Step4Publish({ artistData, setArtistData, shouldSubmit, setShouldSubmit }: { artistData: any, setArtistData: (data: any) => void, shouldSubmit: boolean, setShouldSubmit: (val: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    // Upload audio file if present and not already a URL
    const uploadAndPostArtist = async () => {
      try {
        let audioUrl = artistData.audio_preview_url;
        if (artistData.audio_preview_file && artistData.audio_preview_file instanceof File) {
          const formData = new FormData();
          formData.append("file", artistData.audio_preview_file);
          const uploadRes = await fetch(`${backendUri}/upload-audio/`, {
            method: "POST",
            body: formData
          });
          if (!uploadRes.ok) throw new Error("Audio upload failed");
          const uploadData = await uploadRes.json();
          audioUrl = uploadData.audio_preview_url;
        }
        const finalArtistData = { ...artistData, audio_preview_url: audioUrl };
        delete finalArtistData.audio_preview_file;
        const response = await fetch(`${backendUri}/artists`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(finalArtistData)
        });
        const data = await response.json();
        console.log("Artist created:", data);
        // Redirect to /artists after successful creation
